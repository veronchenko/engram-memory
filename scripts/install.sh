...
if [ "$HAS_CLAUDE" -eq 1 ] && [ "$HAS_CODEX" -eq 1 ]; then
    if [ -t 0 ]; then
        echo ""
        read -r -p "Both claude and codex found on PATH. Install for which? [claude/codex/both] (default: both): " SELECTION
        SELECTION="${SELECTION:-both}"
        case "$SELECTION" in
            claude) HAS_CODEX=0 ;;
            codex) HAS_CLAUDE=0 ;;
            both) ;;
            *) echo "warn: unrecognized selection '$SELECTION' — defaulting to both" >&2 ;;
        esac
    else
        echo "Non-interactive shell — installing for both detected CLIs."
    fi
fi
...
if [ "$HAS_CODEX" -eq 1 ]; then
    ...
    cp -f "$PLUGIN_SOURCE/agents/engram-project-onboarder.codex.toml" "$CODEX_AGENTS_DIR/engram-project-onboarder.toml"
    echo "Installed engram-project-onboarder subagent -> $CODEX_AGENTS_DIR/engram-project-onboarder.toml"
fi
...
if [ "$HAS_CODEX" -eq 1 ]; then
    ...
    BEGIN_MARKER="<!-- BEGIN engram-memory -->"
